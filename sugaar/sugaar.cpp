...
inline Vec4 adjust_color(Vec4 color, int samples_per_pixel = 1, double gamma = 2.0) {
	Vec4 c;
	c[0] = pow(color[0] / (double)samples_per_pixel, 1 / gamma);
	c[1] = pow(color[1] / (double)samples_per_pixel, 1 / gamma);
	c[2] = pow(color[2] / (double)samples_per_pixel, 1 / gamma);
	c[3] = pow(color[3] / (double)samples_per_pixel, 1 / gamma);
	return c;
}
...
int main() {
	...
	double aperture = 0.1;
	Camera cam(lookfrom, lookat, vup, pi/9, aspect_ratio, aperture, dist_to_focus);
	...
	std::cout << "Finished rendering a " << width << "x" << height << " (" << samples_per_pixel << " samples per pixel) image in " << duration.count()/1000.0 << " seconds.\n" << std::endl;
